...
var templates = map[string]string{
	...
	{{- if eq .Kind "paragraph" -}}
		{{- exec "text" .Text | paragraph -}}
	{{- else if eq .Kind "code" -}}
		{{- exec "text" .Text | codeBlock -}}
	{{- else if eq .Kind "header" -}}
		{{- exec "text" .Text | header .Level -}}
...
{{- range .Types -}}
	...
	{{- range .Methods -}}
		{{- if .Receiver -}}
			{{- codeHref .Location | link (escape .Name) | printf "func \\(%s\\) %s" (escape .Receiver) | localHref | link (escape .Signature) | listEntry 1 -}}
		{{- else -}}
			{{- codeHref .Location | link (escape .Name) | printf "func %s" | localHref | link .Signature | listEntry 1 -}}
		{{- end -}}
	{{- end -}}
...
`,
	...
	{{- else if eq .Kind "docLink" -}}
		{{- docLink (exec "text" .Inner) .Path (localHref .Href) -}}
	{{- end -}}
{{- end -}}
